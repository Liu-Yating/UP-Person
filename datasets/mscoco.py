...
class MSCOCO(BaseDataset):
    ...
    def __init__(self, root='', verbose=True):
        ...
        self.anno_path_train = op.join(self.dataset_dir, 'coco_train.json')
        self.anno_path_test = op.join(self.dataset_dir, 'coco_test.json')
        self.anno_path_val = op.join(self.dataset_dir, 'coco_val.json')
        ...
        if verbose:
            self.logger.info("=> MSCOCO Images and Captions are loaded")
    ...
    def _process_anno(self, annos: List[dict], training=False):
        ...
        if training:
            dataset = []
            for anno in annos:
                image_id = int(anno['image_id'].split("coco_")[1])  
                pid = image_id
