...
def get_parameter(model):
    ...
    #print("Total Param: {:.2f}M".format(total), "Trainable Param: {:.2f} M".format(trainable) )
...
if __name__ == '__main__':
    ...
    #name_no_update = ["base_model.transformer", "base_model.token_embedding", "base_model.ln_final.weight", "base_model.token_embedding", "base_model.positional_embedding"]
    for name, param in model.named_parameters():
    ...
    name_update =  ["adapter", "scale_1", "scale_2", "prefix_learner", "scale_to_grad"]
    for name, param in model.named_parameters():
